...
{id:'earlyExit',pattern:/forfeit|penalt|early departure/i,title:'Leaving early could cost you',level:'attention',explanation:'This passage describes a potential cost for leaving early. Check the trigger, amount and exceptions. Enforceability needs professional review.',question:'When does the early-exit charge apply, and are there exceptions?'},
{id:'discretion',pattern:/solely|sole discretion|unilateral/i,title:'One party controls the decision',level:'attention',explanation:'This wording gives one party discretion. Ask for objective criteria and supporting records before agreeing.',question:'Can deductions require itemised receipts and an agreed process?'},
{id:'noticePeriods',pattern:/notice/i,title:'Check the notice periods',level:'clarify',explanation:'Check how much notice each party must give and how it must be delivered. Different periods are not automatically unlawful.',question:'What notice must each party give, and how should it be delivered?'},
{id:'undefinedResponsibility',pattern:/not defined|minor repairs/i,title:'A responsibility needs clearer wording',level:'clarify',explanation:'Broad or undefined terms can make responsibilities hard to understand. Ask for examples and limits.',question:'Can this responsibility be defined with examples and a cost limit?'},
{id:'recurringCost',pattern:/monthly|per month/i,title:'Check the full recurring cost',level:'info',explanation:'Review recurring payments, extra charges and due dates together. This topic flag does not establish that the terms are safe.',question:'What additional charges can be added, and when are payments due?'}
].flatMap(r=>{const i=ps.findIndex(p=>r.pattern.test(p));return i<0?[]:[{id:r.id,title:r.title,level:r.level,explanation:r.explanation,question:r.question,quote:ps[i],source:`Passage ${i+1}`}];});}
